...
import { lydiaXState } from '../public/L4LydiaXState.js'

var exampleMachineCode = lydiaXState;

const initialMachineCode = exampleMachineCode;
...
export const sourceModel = createModel(
  ...
  },
  {
    events: {
      ...
      LOADED_FROM_LOCAL: (rawSource: string) => ({
        rawSource,
      }),
    ...
    },
    {
      ...
      actions: {
        ...
        parseQueries: assign((ctx) => {
          ...
          if (queries.get('local')) {
            return {
              sourceID: queries.get('local'),
        ...
        loadSourceContent: (ctx) => async (send) => {
          switch (ctx.sourceProvider) {
            case 'local':
              ...
              const localSource: string = await localResponse.text();;
